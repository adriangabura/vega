...
@attr.define
class BasicConfig:
    APP_ENV: str = attr.field(factory=StringPair.from_env_var('APP_ENV', default='local'))
    AWS_ACCESS_KEY_ID: str = attr.field(factory=StringPair.from_env_var('AWS_ACCESS_KEY_ID'))
    AWS_SECRET_ACCESS_KEY: str = attr.field(factory=StringPair.from_env_var('AWS_SECRET_ACCESS_KEY'))
    AWS_REGION: str = attr.field(factory=StringPair.from_env_var('AWS_REGION'))
    BUCKET_NAME: str = attr.field(factory=StringPair.from_env_var('BUCKET_NAME'))
    PRESIGNED_URL_EXPIRATION: int = attr.field(factory=lambda: from_env_var('PRESIGNED_URL_EXPIRATION'), converter=int)
